...
const EvalQuery = () => {
  ...
  async function queryEvals(e) {
    ...
    if (preferredTutorList.length !== 0 && preferredTutorList.length < tutors.length) {
      studentQueryConditions.push(
        where('preferred_tutor', 'in', preferredTutorList.map(t => t.id))
      );
    }
    ...
    if (schoolMatching === 'exact') {
      ...
      studentQueryConditions.push(
        where('student_school', '==', studentSchool)
      );
    }
    ...
    if (sourceMatching === 'exact') {
      ...
      studentQueryConditions.push(
        where('student_source', '==', studentSource)
      );
    }
    ...
    if (tutorList.length !== 0 && tutorList.length !== tutors.length) {
      evalQueryConditions.push(
        where('tutor_id', 'in', tutorList.map(t => t.id))
      );
    }

    
    if (evalQueryConditions.length === 0) {
      if (!window.confirm("You either specified no filters, or are using filters that can only be applied client-side. This will query every evaluation in the database. Proceed?")) {
        setLoading(false);
        return;
      }
    }
  ...
  const TutorDropdownToggle = React.forwardRef(({ style, className, onClick, value }, ref) => (
    <Form.Control
      ref={ref}
  ...
  const TutorDropdown = React.forwardRef(({ style, className, value, valueSetter }, ref) => {
  ...
  return (
    <div className="d-flex flex-column p-3">
      ...
      <Card className="p-3 bg-light-subtle">
        ...
        <Row>
          <Col>
            ...
            <InputGroup>
              <Dropdown>
                <Dropdown.Toggle variant="secondary">{dateMatchingLabel()}</Dropdown.Toggle>
                <Dropdown.Menu>
                  <Dropdown.Item onClick={() => setDateMatching('on')}>On</Dropdown.Item>
                  <Dropdown.Item onClick={() => setDateMatching('before')}>On or Before</Dropdown.Item>
                  <Dropdown.Item onClick={() => setDateMatching('after')}>On or After</Dropdown.Item>
                  <Dropdown.Item onClick={() => setDateMatching('between')}>Between</Dropdown.Item>
                </Dropdown.Menu>
              </Dropdown>
          ...
          <Col>
            ...
            <InputGroup>
              <Button
                as={MatchingType}
                onClick={() => subjectMatching === 'like' ? setSubjectMatching('exact') : setSubjectMatching('like')}
              >
        ...
        <Row>
          <Col>
            ...
            <InputGroup>
              <Button
                as={MatchingType}
                onClick={() => studentNameMatching === 'like' ? setStudentNameMatching('exact') : setStudentNameMatching('like')}
              >
          ...
          <Col>
            ...
            <InputGroup>
              <Dropdown>
                <Dropdown.Toggle as={TutorDropdownToggle} value={preferredTutorList} />
                <Dropdown.Menu as={TutorDropdown} value={preferredTutorList} valueSetter={setPreferredTutorList} />
              </Dropdown>
            </InputGroup>
          </Col>
        </Row>
        ...
        <Row>
          <Col>
            ...
            <InputGroup>
              <Button
                as={MatchingType}
                onClick={() => schoolMatching === 'like' ? setSchoolMatching('exact') : setSchoolMatching('like')}
              >
          ...
          <Col>
            ...
            <InputGroup>
              <Button
                as={MatchingType}
                onClick={() => sourceMatching === 'like' ? setSourceMatching('exact') : setSourceMatching('like')}
              >
          ...
          <Col>
            ...
            <InputGroup>
              <Button
                as={MatchingType}
                onClick={() => gradeMatching === 'like' ? setGradeMatching('exact') : setGradeMatching('like')}
              >
          ...
          <Col>
            ...
            <InputGroup>
              <Dropdown>
                ...
                <Dropdown.Menu as={TutorDropdown} value={tutorList} valueSetter={setTutorList} />
              </Dropdown>
            </InputGroup>
          </Col>
          <Col></Col>
      ...
      <div className="d-flex justify-content-center">
        {loading ? 
        <Button className="m-3 w-25" type='submit' id='queryButton'>Query <span className='spinner-border spinner-border-sm' /></Button>
        :
        <Button className="m-3 w-25" type='submit' id='queryButton'>Query</Button>
        }
      </div>
      </Form>
      {loading ?

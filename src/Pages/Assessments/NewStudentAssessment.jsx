...
const NewStudentAssessment = () => {
  ...
  useEffect(() => {
    ...
    let newAssessment = assessments[selectedGrade].find(
    ...
    if (newAssessment?.file === "" || !newAssessment?.file)
      return setSelectedAssessment(newAssessment);

    getDownloadURL(ref(storage, newAssessment?.file)).then((url) => {
  ...
  function handleSubmit(e) {
    ...
    if (completedAssessmentFile) {
      const completedAssessmentRef = ref(
        storage,
        `student_assessments/${student.id}/${completedAssessmentFile.name}`,
      );
      uploadBytes(completedAssessmentRef, completedAssessmentFile).then(() => {
        addDoc(collection(db, "student_assessments"), {
          ...selectedAssessment,
          student_id: student.id,
          student_name: student.student_name,
          issued_by: selectedTutor,
          issued_by_name: tutors.find((t) => t.id === selectedTutor)
            .displayName,
  ...
  function categoryOptions() {
    ...
    return assessments[selectedGrade].map((assessment) => (
      <option
        key={`${assessment.grade}:${assessment.category}`}

...
const StudentProfile = () => {
  ...
  function handleSubmit(e) {
    ...
    document.getElementById("update").innerHTML =
      "Updating <span class='spinner-border spinner-border-sm' />";
    ...
    setDoc(doc(studentRef.current, "standards", selectedStandard.id), {
      ...
      setShow(false);
      addToast({
        header: "Standard Progress Updated",
        message: `${student.student_name}'s progression for Standard ${selectedStandard.key} has been successfully updated.`,
      });
    });
  }
  ...
  function handleRemove(e) {
    ...
    document.getElementById("remove").innerHTML =
      "Removing <span class='spinner-border spinner-border-sm' />";
    document.getElementById("remove").setAttribute("disabled", true);

    deleteDoc(doc(studentRef.current, "standards", selectedStandard.id)).then(
      () => {
        setShow(false);
        addToast({
          header: "Standard Progress Removed",
          message: `No longer tracking ${student.student_name}'s progression for ${selectedStandard.key}`,
        });
      },
    );
  }
          ...
          <Can do='manage' on='students'>
            <div className='d-flex justify-content-end'>
              <button
                ...
                onClick={() =>
                  navigate(`/students/edit/${studentRef.current.id}`)
                }
              >
                Make Changes
              </button>
  ...
  return (
    <div className='p-3 d-flex flex-column'>
      ...
      >
        <Offcanvas.Header>
          <Offcanvas.Title>
            Standard {selectedStandard ? selectedStandard.key : ""}
          </Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          {selectedStandard ? (
            <div className='d-flex flex-column'>
              <Form onSubmit={handleSubmit}>
                ...
                <Button className='m-3' type='submit' id='update'>
                  Update Progression
                </Button>
                <div className='fs-6 fst-italic text-end'>
                  Last updated{" "}
                  {dayjs
                    .unix(selectedStandard.timestamp.seconds)
                    .format("MM/DD/YYYY")}{" "}
                  (
                  {dayjs
                    .duration(
                      dayjs().diff(
                        dayjs.unix(selectedStandard.timestamp.seconds),
                      ),
                    )
                    .humanize()}{" "}
                  ago)
